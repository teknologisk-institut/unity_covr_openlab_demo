...
public class ZEDControllerTracker : MonoBehaviour
{
    ...
    protected virtual void Awake()
    {
        ...
        if (!zedManager)
        {
            ...
            if (ZEDManager.GetInstances().Count > 1)
            {
                ...
                UnityEngine.Debug.Log("Warning: ZEDController automatically set itself to first available ZED (" + zedManager.cameraID + ") because zedManager " +
    ...
    public Vector2 GetAxis(EVRButtonId buttonid)
    {
        ...
        if (axis < 0 || axis > 4)
        {
            UnityEngine.Debug.LogError("Called GetAxis with " + buttonid + ", which is not an axis.");
            return Vector2.zero;
        }
