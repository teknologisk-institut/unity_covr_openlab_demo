...
public class ZEDManager : MonoBehaviour
{
   ...
   //private int arlayer = 30;
    ...
    void Awake()
    {
        ...
        if (!res)
        {
            UnityEngine.Debug.LogError("ZEDManager on " + gameObject.name + " couldn't connect to camera: " + cameraID +
...
#region INITIALIZATION
    ...
    private System.Collections.IEnumerator InitZED()
    {
        ...
        if (lastInitStatus == sl.ERROR_CODE.SUCCESS)
        {
            ...
            {
                UnityEngine.Debug.LogWarning(ZEDLogMessage.Error2Str(ZEDLogMessage.ERROR.SCREEN_RESOLUTION));
            }
    ...
    void setRenderingSettings()
    {
        ...
        if (maincam != null)
        {
            ...
            {
                UnityEngine.Debug.LogError("[ZED Plugin] Only Forward and Deferred Shading rendering path are supported");
...
#region IMAGE_ACQUIZ
    ...
    private void AcquireImages()
    {
        if (requestNewFrame && zedReady)
        {
            ...
            lock (zedCamera.grabLock)
            {
                ...
                {
                    UnityEngine.Debug.Log("Camera not detected or disconnected.");
                    isDisconnected = true;
                    Thread.Sleep(10);
    ...
    private void ZEDReady()
    {
        ...
        else if (estimateInitialPosition)
        {
            ...
            if (err != sl.ERROR_CODE.SUCCESS)
                UnityEngine.Debug.LogWarning("Failed to estimate initial camera position");
        }
        ...
        if (isStereoRig && hasXRDevice())
        {
            ...
            if (!zedCamera.IsHmdCompatible && zedCamera.IsCameraReady)
                UnityEngine.Debug.LogWarning("WARNING: AR Passtrough with a ZED is not recommended. Consider using ZED Mini, designed for this purpose.");
        }
        else
        {
    ...
    void EnableTrackingThreaded()
    {
        lock (zedCamera.grabLock)
        {
            ...
            {
                UnityEngine.Debug.Log("Specified path to .area file '" + pathSpatialMemory + "' does not exist. Ignoring.");
                pathSpatialMemory = "";
            }
                ...
                if (zedCamera.GetSVOPosition() >= maxSVOFrame - (svoRealTimeMode ? 2 : 1))
                {
                    ...
                    if (enableTracking)
                    {
                        if (!(enableTracking = (zedCamera.ResetTracking(initialRotation, initialPosition) == sl.ERROR_CODE.SUCCESS)))
                        {
                            ...
                            UnityEngine.Debug.LogError("ZED Tracking disabled: Not available during SVO playback when Loop is enabled.");
                        }
                    }
...
#region OBJECT_DETECTION
    ...
    private IEnumerator startObjectDetection()
    {
        ...
        {
            UnityEngine.Debug.LogError("Tried to start Object Detection while it was already starting. Do you have two scripts trying to start it?");
            yield break;
        }
        if(objectDetectionRunning)
        {
            UnityEngine.Debug.LogWarning("Tried to start Object Detection while it was already running.");
        }

        if (zedCamera != null)
        {
            odIsStarting = true;
            UnityEngine.Debug.LogWarning("Starting Object Detection. This may take a moment.");
            ...
            if (err == sl.ERROR_CODE.SUCCESS)
            {
                UnityEngine.Debug.Log("Object Detection module started in " + watch.Elapsed.Seconds + " seconds.");
                objectDetectionRunning = true;
            }
            else
            {
                UnityEngine.Debug.Log("Object Detection failed to start. (Error: " + err + " )");
...
#region MIRROR
    ...
    public void InitVideoSettings(VideoSettingsInitMode mode)
    {
        ...
        {
            UnityEngine.Debug.LogError("Tried to apply camera settings before ZED camera was ready.");
            return;
        }
        switch(mode)
        {
...
#region EventHandler
    ...
    public void SetMaxDepthRange(float newVal)
    {
        ...
        {
            UnityEngine.Debug.LogWarning("Tried to set max depth range to " + newVal + "m. Must be within 0m and 40m.");
    ...
    private void SetFloatValueOnPlaneMaterials(string propertyname, float newvalue)
    {
        foreach (ZEDRenderingPlane renderPlane in GetComponentsInChildren<ZEDRenderingPlane>())
        {
            ...
            {
                UnityEngine.Debug.LogError("Can't set " + propertyname + " value  for Rendering Path " + renderPlane.ActualRenderingPath +
