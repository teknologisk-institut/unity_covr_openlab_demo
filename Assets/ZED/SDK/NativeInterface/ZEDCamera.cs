...
namespace sl
{
    ...
    public class ZEDCamera
    {
        ...
        */
        ...
        {
            UnityEngine.Debug.Log("[ZED plugin]: " + message);
        }
        ...
        public static bool CheckPlugin()
        {
            try
            {
                ...
                if (res!= 0)
                {
                    UnityEngine.Debug.Log("Error : " + res);
                    //0 = installed SDK is compatible with plugin. 1 otherwise.
                    UnityEngine.Debug.LogError(ZEDLogMessage.Error2Str(ZEDLogMessage.ERROR.SDK_DEPENDENCIES_ISSUE));
                    return false;
                }
            }
            catch (DllNotFoundException) //In case could not resolve the dll/.so
            {
                UnityEngine.Debug.Log("DllNotFoundException");
                UnityEngine.Debug.LogError(ZEDLogMessage.Error2Str(ZEDLogMessage.ERROR.SDK_DEPENDENCIES_ISSUE));
                return false;
            }
